...
const PROVIDER_LABELS: Record<CalendarConnectionView["provider"], string> = {
...
const PROVIDER_MARKS: Record<CalendarConnectionView["provider"], () => React.JSX.Element> = {
  google: GoogleMark,
  outlook: OutlookMark,
};
...
const OUTCOME_MESSAGES: Record<string, { tone: "ok" | "error"; text: string }> = {
  ...
  not_configured: {
    tone: "error",
    text: "Questo calendario non è ancora configurato su questo ambiente.",
  },
...
export function ExternalCalendarSync() {
  ...
  return (
    <section className="rounded-xl border border-border bg-card p-4 md:p-5">
      ...
      {isLoading ? (
        <div className="mt-4 flex items-center justify-center rounded-lg border border-border p-6">
          <Loader2 className="h-5 w-5 animate-spin text-muted-foreground" />
        </div>
      ) : (
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {(connections ?? []).map((connection) => {
            const Mark = PROVIDER_MARKS[connection.provider];
            const isConnected = connection.accountEmail !== null;
            ...
            return (
              ...
              >
                ...
                {isConnected ? (
                  <>
                    <p className="flex items-start gap-1.5 text-xs text-status-qualified">
                      ...
                      <span className="min-w-0 break-all">
                        Connesso come {connection.accountEmail}
                      </span>
                    </p>
                    <button
                      type="button"
                      onClick={() => disconnect(connection.provider)}
                      disabled={pending === connection.provider}
                  ...
                  <>
                    <p className="text-xs text-muted-foreground">Non connesso</p>
                    {/* Un link e non una fetch: il consenso OAuth è una
                        navigazione verso il fornitore, e intercettarla in
                        JavaScript la romperebbe dentro un iframe o con i
                        popup bloccati. */}
                    <a
                      href={`/api/calendar/${connection.provider}/connect`}
                    ...
                    >
                      <Mark />
                      Connetti {connection.provider === "google" ? "Google Calendar" : "Microsoft Outlook / Office 365"}
                    </a>
                  </>
                ) : (
                  <p className="flex items-start gap-1.5 text-xs text-muted-foreground">
                    <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" aria-hidden="true" />
                    Non ancora configurato su questo ambiente.
                  </p>
                )}
              </div>
            );
