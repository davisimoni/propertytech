...
**Quello che manca e' la strada del file.** Senza `STORAGE_*` configurate un video finirebbe come data URI in PostgreSQL, e l'indirizzo dato a Meta sarebbe una nostra funzione che restituisce base64: per un video, un rifiuto a pubblicazione avviata. Inoltre il file passa nel corpo JSON di una funzione serverless, che accetta **4,5 MB** di richiesta: col +33% del base64 restano circa 3 MB (`MAX_VIDEO_BYTES`), cioe' una clip di dieci secondi, non un Reel. Il pannello quindi **chiede alla rotta cosa sa accettare** (`GET /api/social/media`) e lo dichiara prima che l'agente scelga il file, invece di far scoprire il limite a chi ha appena finito di scrivere il testo. Il numero sale quando il caricamento andra' diretto al bucket con un indirizzo prefirmato: e' l'unico pezzo che resta, ed e' anche il momento in cui servira' un bucket **in UE** (§5).
