...
**Commenti Instagram** (`lib/social/comments.ts`, scheda *Commenti Instagram* in `/social`). Leggere i commenti dei post, rispondere e nascondere, con il permesso `instagram_manage_comments`. Stesso gate del resto del modulo: Enterprise, `checkFeatureAccess("socialMultiplier")` — i commenti sono la coda del Social Multiplier, non una funzione con un piano proprio.
...
- **`instagram_manage_comments` è in `META_SCOPES` da settembre 2026**: i token generati prima non lo portano e ricevono un errore sui soli commenti. Per questo gli errori del modulo dicono di ricollegare la Pagina invece di proporre un generico "riprova".
