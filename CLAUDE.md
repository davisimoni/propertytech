...
**Note vocali in arrivo.** Un cliente che risponde a voce è la norma su WhatsApp, quindi i vocali si trascrivono e proseguono nella qualificazione come un messaggio scritto. Vale per tutti i canali con media: Cloud API (`message.type === "audio"`, media scaricato dal Graph), Twilio (`MediaUrl0`/`MediaContentType0`, scaricato con Account SID e Auth Token dell'agenzia) e QR (audio già nel payload del microservizio). Il relay generico riceve solo testo, per costruzione.

- **Un solo percorso di trascrizione** (`lib/whatsapp/voice-note.ts`): tetto a `MAX_WEBHOOK_AUDIO_BYTES` (6 MB, più basso del limite del fornitore perché il cliente sta aspettando in chat), timeout `STT_WEBHOOK_TIMEOUT_MS`, e `cleanTranscript` che scarta le frasi inventate da Whisper sugli audio silenziosi — qualificare un lead su parole mai dette è peggio che non trascrivere.
- **Il cliente riceve sempre una risposta**: servizio non configurato, audio troppo lungo, vocale senza parlato ed errore tecnico hanno ciascuno il proprio messaggio. Il silenzio convince che il numero non sia attivo.
- **Indirizzi dei media su lista di host** (`lib/whatsapp/twilio-media.ts`): l'URL arriva dentro un webhook, e le credenziali dell'agenzia si inviano solo al dominio Twilio, mai al redirect firmato che segue.
- **Senza provider Speech-to-Text** (`STT_API_URL`/`STT_API_KEY`, oppure `OPENAI_API_KEY`) i vocali ricevono la risposta di cortesia: lo stato è visibile nel pannello WhatsApp (`transcriptionReady`), perché è l'unico posto dove l'agenzia se ne accorge prima che accada.

**Video negli allegati: pubblicazione pronta, caricamento no.** La logica Meta per i video esiste ed e' completa in `lib/social/meta.ts`: su Instagram un video singolo e' un **Reel** (`media_type: REELS`, non esiste piu' il video nel feed come tipo a se'), dentro un carosello e' `VIDEO`, e in entrambi i casi il contenitore **va atteso** — Instagram scarica e transcodifica, e `media_publish` chiamata prima risponde con un errore che sembra un rifiuto del contenuto mentre e' solo fretta (`attendiContenitore`, tetto 3 minuti). Su Facebook il video ha un endpoint suo, `/{page-id}/videos` con `file_url`, e **non convive con le foto** nello stesso post: `attached_media` accetta foto non pubblicate, non video, quindi un post misto diventerebbe due post da un gesto solo e l'agente ne vedrebbe uno credendo perso l'altro. La rotta di pubblicazione dichiara `maxDuration = 300`: col minuto di prima la funzione moriva a meta' attesa, lasciando un video elaborato da Meta e nessun post pubblicato.

**Il tipo di ogni allegato lo decide il server** (`lib/social/media-resolve.ts`), non il browser: da quel dato dipende quale endpoint viene chiamato, e senza object storage un allegato vive su `/api/social/media/<id>`, che di estensione non ne ha — il tipo sta nella riga `SocialMediaAsset`, letta filtrando su `organizationId`.

**Il file dei video non passa dalle nostre funzioni.** Va diretto al bucket con un indirizzo prefirmato (`presignPutUrl` in `lib/storage/object-storage.ts`, emesso da `POST /api/social/media/presign`): il corpo di una funzione serverless si ferma a 4,5 MB e un data URI gonfia i byte di un terzo, quindi da quella strada passavano dieci secondi di clip. Il permesso firmato **non consente tre cose**, e sono il motivo per cui e' sicuro: scegliere dove scrivere (la chiave la genera la rotta dentro il prefisso dell'agenzia), superare la dimensione dichiarata (`content-length` entra nella firma, quindi il fornitore rifiuta un file piu' grande — verificato: HTTP 403) e restare valido (scade in quindici minuti). Le foto continuano a passare dalla rotta JSON: mezzo megabyte ci sta comodo, e due strade separate tengono per ciascuna il limite vero invece del minimo fra i due. `scripts/verifica-storage.ts` prova entrambe contro il bucket reale, compreso il rifiuto del file sovradimensionato.

**Commenti Instagram e Facebook** (`lib/social/comments.ts`, scheda *Commenti Social* in `/social`). Leggere i commenti dei post, rispondere e nascondere, su entrambe le piattaforme, con un selettore invece di due schede: per l'agente è lo stesso gesto, e cambia solo quale endpoint di Meta lo esegue. Le differenze vere sono tre e stanno tutte nel modulo: l'oggetto di partenza (media Instagram contro post della Pagina), il percorso per rispondere (`/replies` contro `/comments`) e quello per nascondere (`hide` contro `is_hidden`). Stesso gate del resto del modulo: Enterprise, `checkFeatureAccess("socialMultiplier")` — i commenti sono la coda del Social Multiplier, non una funzione con un piano proprio.

- **L'AI scrive la bozza, l'agente pubblica.** `lib/ai/comment-reply.ts` riempie la casella; a spedire è sempre un clic umano. Una risposta finisce sotto un post pubblico, a nome dell'agenzia, e non si richiama prima che qualcuno la legga: non è un'azione da affidare a un modello senza revisione. Il registro è il **"lei"** anche quando il commento dà del tu (§1): lì parla l'agenzia a uno sconosciuto, non noi al nostro utente.
- **Il disclaimer AI sta nella schermata, non nel commento pubblicato.** Stessa scelta dei post social generati: il testo che esce è rivisto da una persona, e una nota sull'AI dentro una risposta pubblica sarebbe incomprensibile a chi la legge. Il disclaimer resta sotto la bozza, dove lo legge chi decide se pubblicarla.
- **Si nasconde, non si cancella.** Nascondere è reversibile e basta contro lo spam; una cancellazione fatta per sbaglio da un telefono no. L'azione "elimina" non è esposta di proposito.
- **Il token è l'isolamento fra agenzie.** Nessun controllo nostro su chi possiede un media: ogni agenzia usa il proprio token e Meta rifiuta gli id che non le appartengono. Un confronto aggiuntivo darebbe l'illusione di una seconda barriera senza esserlo.
- **I permessi sono tre, e in `META_SCOPES` da settembre 2026**: `instagram_manage_comments` per Instagram; per Facebook **due distinti**, perché Meta separa il leggere dal rispondere — `pages_read_user_content` per i commenti scritti *da altri* sotto i post della Pagina (`pages_read_engagement` da solo legge i contenuti della Pagina, non quelli dei visitatori) e `pages_manage_engagement` per rispondere e nascondere a nome della Pagina. I token generati prima non li portano e ricevono un errore sui soli commenti: per questo gli errori del modulo dicono di ricollegare la Pagina invece di proporre un generico "riprova".
- **Il selettore di piattaforma compare anche in errore.** Un'agenzia senza account Instagram collegato vede l'errore di Instagram: se il selettore vivesse solo nella schermata riuscita resterebbe chiusa fuori da Facebook, che invece funziona. L'uscita da un errore non deve essere ricaricare la pagina.
...
- **Ricarica una tantum: Starter, Professional ed Enterprise senza consumo attivo** (annuale, beta tester assegnati a mano). Pacchetto da `EXTRA_CREDITS_PACK_SIZE` (100) conversazioni, Checkout in `mode: "payment"` (`STRIPE_PRICE_ID_EXTRA_CREDITS`), accreditato dal webhook su `Organization.bonusWhatsappCredits` con idempotenza su `CreditRecharge`. Il pulsante compare dall'80% di utilizzo e solo al titolare. La regola è `canRechargeCredits(planId, overageActive)`, usata sia dalla rotta sia dal pulsante: senza il pacchetto un Enterprise annuale resterebbe fermo fino al mese dopo, con il consumo attivo pagherebbe in anticipo ciò che gli verrebbe addebitato solo usandolo.
- **Il pacchetto resta finché non si usa.** Si consuma solo con le conversazioni oltre la dotazione del piano: a fine mese `bonusConsumato()` scala il bonus di quelle, il resto passa al mese dopo. Prima il bonus non scendeva mai, e un pacchetto da 100 alzava il limite ogni mese per sempre.
...
### Mese di consumo — ancorato, non guidato dagli eventi

I contatori ripartono ogni mese alla data di `Subscription.billingCycleAnchor` (la `billing_cycle_anchor` di Stripe; per i piani assegnati a mano, la data di assegnazione), **anche sui piani annuali**. Logica in `lib/billing/usage-period.ts`.

- **Chiusura al primo accesso.** `getUsageStats` e `incrementUsage` chiamano `assicuraPeriodoCorrente()`: se il mese calcolato dall'ancora ha superato `UsageTracker.periodStart`, azzera contatori (Radar compreso) e memoria degli avvisi e scala il bonus consumato. Non si aspetta nessun evento Stripe.
- **Il webhook non azzera più a ogni `subscription.updated`.** Prima ogni aggiornamento — una disdetta programmata, una postazione in più — regalava una dotazione intera a metà mese, e un annuale riceveva le conversazioni "mensili" una volta l'anno. Ora il webhook azzera solo al **cambio di piano** e registra l'ancora; `datiCambioPiano()` è condiviso con `scripts/assegna-piano.ts`.
- **Calendario come Stripe.** Mesi calcolati sempre dall'ancora originale: un'ancora al 31 cade il 28/29 febbraio e torna al 31 a marzo.
...
Lavorano sull'**acquisizione dell'incarico**, che i quattro moduli non coprono, e **non consumano crediti**: restano disponibili anche a contatori esauriti. Aggiungere un contatore qui significherebbe inventare una valuta che il listino non dichiara, scoperta dall'agente quando smette di funzionare qualcosa che credeva incluso.

Quasi tutto gira nel browser, senza modello, e non è una scorciatoia: sono frasi scritte e regole fisse, e farle generare darebbe ogni volta un testo diverso dove serve invece quello che in agenzia funziona. Il Report di Valorizzazione riusa la pipeline PDF esistente (`lib/pdf/client.ts` + branding dell'agenzia) e **non** porta `AI_DISCLAIMER`: il contenuto lo scrive l'agente, e un avviso fuori posto toglie peso a quello sui documenti che lo richiedono davvero.

**Le due eccezioni che chiamano il modello**, entrambe perché dipendono da una combinazione di dati che nessun elenco fisso può contenere:

- **Obiezioni su misura** nel Kit Script (`lib/ai/objections.ts`, `POST /api/bonuses/objections`). Da tre a cinque obiezioni di **chi compra** su uno specifico immobile, con script WhatsApp e strategia. L'immobile arriva dal portafoglio (dati riletti dal database, non dal corpo della richiesta: è ciò che garantisce che siano dell'agenzia e che siano veri) oppure scritto a mano. La regola che regge tutto: **solo ciò che è scritto**. Un immobile senza classe energetica indicata non è "probabilmente una G", e un piano non dichiarato non diventa "senza ascensore": un'obiezione inventata manda l'agente a giustificarsi di un problema che il cliente non aveva sollevato, cioè a crearlo. Senza immobile restano le obiezioni classiche, statiche, senza attesa. Gli otto script del **proprietario** restano scritti a mano e separati: sono un interlocutore diverso, e usare la risposta sbagliata col cliente sbagliato si sente.
- La **Checklist** resta senza AI in ogni sua parte, ma salva su `PropertyAudit` e produce un PDF (vedi Modulo 3). Non certifica nulla e lo dichiara in pagina, per la stessa ragione per cui il Fascicolo documentale non è annunciato come antiriciclaggio.

`AI_DISCLAIMER` compare **in schermata sopra le obiezioni generate**, non dentro lo script che l'agente incollerà su WhatsApp: a leggerlo dev'essere chi decide se inviare, non il cliente che riceve. Stessa scelta già fatta per i post social e per le risposte ai commenti.
...
Nel **Portale Clienti** sono abilitati `price` e `promotion_code`: il cambio piano si fa lì, con quantità **non** modificabile — Stripe accende `adjustable_quantity` da sola, e senza spegnerla un'agenzia potrebbe portare a cinque la quantità del proprio piano pagandolo cinque volte. **Upgrade immediato** con conguaglio fatturato subito (`always_invoice`); **downgrade e passaggio da annuale a mensile a fine periodo** (`schedule_at_period_end`: `decreasing_item_amount`, `shortening_interval`), così l'agenzia tiene fino alla scadenza limiti e bonus già pagati. La configurazione è **nel codice** (`lib/billing/portal-config.ts`) e si riallinea alla prima apertura del portale di ogni istanza: una modifica fatta a mano in dashboard viene sovrascritta, e va fatta lì.
...
Altre tre regole, aggiunte dopo:

- **Si leggono le voci applicate, mai `pending_update`.** `customer.subscription.created` e `.updated` passano dalla stessa `sincronizzaAbbonamento`: un upgrade il cui pagamento è ancora in corso non cambia le voci finché non è incassato, quindi non sblocca nulla prima del tempo; un downgrade programmato lascia il prezzo pagato fino al cambio di fase, e il piano scende quando arriva l'evento con il prezzo nuovo.
- **L'attivazione del piano è una scrittura condizionata.** `checkout.session.completed` e `customer.subscription.created` arrivano quasi insieme: letti in parallelo, entrambi vedevano "trial" e azzeravano contatori, scalavano crediti bonus ed email due volte. `activatePlan` scrive il nuovo piano solo se lo stato è ancora quello letto (`updateMany` con lo stato atteso), e solo chi lo cambia davvero applica gli effetti.
- **Per decidere cosa sbloccare si legge il piano dal database** (`pianoCorrente`/`getPlanId`), mai `session.user.planId`: il token fotografa il piano al login e il webhook non può aggiornare i cookie di chi è collegato. Con il token un'agenzia scesa di piano teneva i bonus fino al logout, e una appena abbonata vedeva il Checkout invece del portale. Il `planId` del token resta per l'intestazione, rinfrescata al ritorno da Checkout e portale (`?checkout=success`, `?portale=ritorno`).
- **Il downgrade programmato si annuncia.** Il portale lo realizza con una *subscription schedule*: il webhook legge la fase successiva e ne scrive piano e data in `Subscription.pendingPlanId` / `pendingPlanAt` (anche dagli eventi `subscription_schedule.*`, perché annullare un downgrade cambia la schedule e non sempre l'abbonamento). Servono **solo all'avviso** "Il tuo piano passerà a Starter il…" su `/settings` e `/bonuses`, dove si elencano anche i bonus che non saranno più inclusi: il piano lo cambia sempre e solo il prezzo, via `activatePlan`. La disdetta a fine periodo (`cancelAtPeriodEnd`) usa lo stesso avviso e ha la precedenza. Si annunciano solo i cambi di **piano**, non il passaggio da annuale a mensile sullo stesso piano.
- **Le migrazioni non le applica Vercel.** La build esegue `prisma generate && next build`, non `migrate deploy`: una migrazione va applicata al database **prima** del push del codice che usa le colonne nuove, o per qualche minuto ogni lettura di quel modello fallisce in produzione. Le colonne nuove vanno aggiunte nullable, così il codice ancora online non se ne accorge.
...
- **Revoca, in quattro vie** (`lib/push/device.ts`, `DELETE /api/push/subscribe`): interruttore in Impostazioni; **logout**, che revoca l'iscrizione del dispositivo prima di uscire (un telefono condiviso non deve mostrare i clienti di chi è uscito); **blocco dal browser**, rilevato alla prima apertura dell'app confrontando l'endpoint ricordato in `localStorage` con l'iscrizione attuale; **404/410** del servizio push all'invio. La `DELETE` accetta la sessione **oppure** il segreto `auth` dell'iscrizione, che conosce solo il dispositivo: revocare un consenso non deve richiedere di essere ancora collegati.
...
- **Disiscrizione**: token HMAC senza scadenza (`unsubscribe-token.ts`); intestazioni `List-Unsubscribe` + `List-Unsubscribe-Post` (RFC 8058, richieste da Gmail e Yahoo); link unico `/api/unsubscribe?token=` nel piè di pagina e nell'intestazione. Il **GET non scrive** — i filtri di sicurezza aziendali (Safe Links, Mimecast, Proofpoint) aprono i link delle email e disiscriverebbero chi non l'ha chiesto — e rimanda a `/newsletter/disiscrizione`, che disiscrive appena si apre nel browser: per la persona è un clic. Da lì si può riattivare con lo stesso token. La **POST** in un clic dei client di posta scrive direttamente. Preference center in Impostazioni → Privacy & Normativa (`/api/account/email-preferences`).
- **Opt-out come data, non come booleano**: `User.newsletterOptOutAt` registra *quando* l'opposizione è stata esercitata, che è ciò che serve a dimostrarla (principio di responsabilizzazione, art. 5.2 GDPR); `null` significa iscritto. Una seconda disiscrizione non sposta la data della prima.
...
- **Gli script CLI (`scripts/`) sono esclusi dal `tsconfig.json`.** `next build` si ferma al primo file che non compila, e un utilitario che nessun cliente esegue ha già bloccato un deploy di produzione per un `argv[i]` possibilmente `undefined`: il codice che serve il cliente non può dipendere dalla compilazione di uno strumento interno. Il prezzo è reale e va conosciuto: quegli script scrivono nel database di produzione e girano con `tsx`, che traspila senza verificare i tipi, quindi un errore si manifesta **a comando avviato**. Per questo validano gli argomenti in ingresso e si fermano prima di scrivere, invece di fidarsi della forma dei dati; e per questo ogni scrittura passa da una transazione o da una guardia esplicita.
